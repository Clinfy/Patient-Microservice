...
Operational guide for agents in this repository (`/extra/IdeaProjects/Patient-Microservice`).

## 1) Project Context

- This microservice is responsible for handling administrative information of the patients needed for handle the attention.
...
> **Note on SDD:** This workflow complements Spec-Driven Development (SDD). When SDD is active, task files in `.agent/tasks` serve as a local, human-readable companion to SDD artifacts persisted in engram. Both can coexist — SDD handles structured specs/design/tasks across sessions, while `.agent/tasks` files provide a quick in-repo reference during implementation.
...
- **ALL written artifacts MUST be in English.** This includes:
    - This file (`CLAUDE.md`) and any agent configuration files.
